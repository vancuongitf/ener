            ...
            <li class="nav-item dropdown" style="position: static;">
                ...
                <div class="dropdown-menu bg-primary" style="width:100%; border: none; border-radius: 0px; padding: 0px; margin: 0px;">
                    <div class="row" style="padding: 0px 15px;">
                        <div class="col-12">
                            <a class="dropdown-item header-text" style="padding: 3.75px 0px; background-color:transparent !important;" href="{{url($tag->route)}}">{{$tag->name}}</a>
                        </div>
                        @foreach ($tag->childs as $tag2)
                        <div class="col-lg-6">
                            <a class="dropdown-item header-text" style="background-color:transparent !important;" href="{{url($tag->route . '/' . $tag2->route)}}">{{$tag2->name}}</a>
                        </div>
                        <div class="col-lg-6">
                            @foreach ($tag2->childs as $tag3)
                            <a href="{{url($tag->route . '/' . $tag2->route . '/' . $tag3->route)}}" class="dropdown-item header-text" style="background-color:transparent !important; margin: 0px 15px;">{{$tag3->name}}</a>                                @endforeach
                        </div>
                        @endforeach
                    </div>
                </div>
